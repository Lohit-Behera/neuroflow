...
import { SerializableFileInfo } from "./OllamaNode";

export const SDImageToImageFileStorage = {
  files: {} as Record<string, File>,

  storeFile: (file: File, id: string): void => {
    SDImageToImageFileStorage.files[id] = file;
  },

  getFile: (id: string): File | null => {
    return SDImageToImageFileStorage.files[id] || null;
  },

  removeFile: (id: string): void => {
    if (SDImageToImageFileStorage.files[id]) {
      delete SDImageToImageFileStorage.files[id];
    }
  },
};

const SDImageToImageNode: React.FC<NodeProps> = ({ data, isConnectable }) => {
  ...
  useEffect(() => {
    ...
    if (file) {
      ...
      SDImageToImageFileStorage.storeFile(file, id);
    } else {
      // Remove file if no longer needed
      SDImageToImageFileStorage.removeFile(id);
    }
...
export default SDImageToImageNode;
